        ...
        try:
            ...
            if password == re_password:
                ...
                if User.objects.filter(username=username).exists():
                    ...
                    response.set_cookie( "sa_token", AuthToken.objects.create(user)[1])
                    return response
            else:
...
class Verify(APIView):
    ...
    permission_classes = (IsAuthenticated,)
    def post(self, request, format=None):
        objs = AuthToken.objects.filter(token_key=request.COOKIES["sa_token"][:CONSTANTS.TOKEN_KEY_LENGTH])
        if len(objs) == 0:
            return None
